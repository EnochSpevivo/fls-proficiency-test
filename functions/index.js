  ...
  return async.auto(
    ...
    (err, results) => {
      ...
      const pageProperties = {
        ...
        partials: {
          preArrivalTest: path.join(
            __dirname,
            "assets/partials/preArrivalTest"
          ),
          proficiencyTest: path.join(
            __dirname,
            "assets/partials/proficiencyTest"
          ),
...
app.post("/pre-arrival-test", (req, res) => {
  ...
  res.status(200).json({
    msg: "successfully submitted pre-arrival test responses",
    payload: req.body,
  });
});
...
app.post("/grade-test", (req, res) => {
  ...
  return jsonfile.readFile(
    path.join(__dirname, "assets/js/proficiency-test-answers.json"),
    (err, testAnswers) => {
      if (err) {
      ...
      if (testResponseData.length !== globals.numTestQuestions)
        return res
          .status(400)
          .send({ err: "Please answer all test questions" });
      ...
      const gradeTest = (testResponseData, testAnswers) => {
        ...
        for (let i = 0; i < testResponseData.length; i++) {
          if (
            testResponseData[i].selectedOptionId ==
            testAnswers[i].correctOptionId
          )
      ...
      const emailTestResults = async () => {
        ...
        const emailHtml = `${generateHtml(
          globals.userData
        )}<h2>Proficiency Test Results</h2><p><strong>Correct responses:</strong> ${testResults}/${
          globals.numTestQuestions
        }</p><p><strong>Level:</strong> ${level}</p><p><strong>Proficiency level:</strong> ${rank}</p>`;
      ...
      return emailTestResults()
        .then((emailResults) => {
          ...
          return res.status(200).json({
            msg: "successfully graded test",
            gradedTest: gradeTest(testResponseData, testAnswers),
          });
        })
        .catch((err) => {
